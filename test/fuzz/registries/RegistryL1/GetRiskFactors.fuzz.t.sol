...
contract GetRiskFactors_RegistryL1_Fuzz_Test is RegistryL1_Fuzz_Test {
    ...
    function testFuzz_Success_getRiskFactors(
        ...
        registry_.setAssetModule(assets[0], address(primaryAM));
        registry_.setAssetModule(assets[1], address(primaryAM));
        vm.startPrank(address(registry_));
        ...
        (uint16[] memory actualCollateralFactors, uint16[] memory actualLiquidationFactors) = registry_.getRiskFactors(
