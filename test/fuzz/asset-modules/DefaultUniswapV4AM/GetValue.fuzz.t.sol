...
contract GetValue_DefaultUniswapV4AM_Fuzz_Test is DefaultUniswapV4AM_Fuzz_Test {
    ...
    function testFuzz_Success_getValue(TestVariables memory vars) public {
        // Given : Valid ticks
    ...
    function testFuzz_Success_getValue_NativeToken(TestVariables memory vars) public {
        // Given : Valid ticks
        (vars.tickLower, vars.tickUpper) = givenValidTicks(vars.tickLower, vars.tickUpper);
        ...
        int24 tickSpacing = 1;
        randomPoolKey =
            initializePoolV4(address(0), address(token1_), sqrtPriceX96, address(validHook), 500, tickSpacing);
        ...
        // And : Add underlying tokens and its oracles to Arcadia.
        addNativeTokenToArcadia(address(0), int256(uint256(vars.priceToken0)));
        addAssetToArcadia(address(token1_), int256(uint256(vars.priceToken1)));
