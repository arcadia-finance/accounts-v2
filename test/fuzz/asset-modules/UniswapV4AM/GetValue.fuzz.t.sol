...
contract GetValue_UniswapV4AM_Fuzz_Test is UniswapV4AM_Fuzz_Test {
    ...
    function testFuzz_Success_getValue_valueInUsd(TestVariables memory vars) public {
        ...
        int24 tickSpacing = 1;
        randomPoolKey =
            initializePool(address(token0), address(token1), sqrtPriceX96, address(validHook), 500, tickSpacing);
    ...
    function testFuzz_Success_getValue_RiskFactors(
        ...
        randomPoolKey = initializePool(
            address(token0), address(token1), TickMath.getSqrtPriceAtTick(0), address(validHook), 500, tickSpacing
        );
