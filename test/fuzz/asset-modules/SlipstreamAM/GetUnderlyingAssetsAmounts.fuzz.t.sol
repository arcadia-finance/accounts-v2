...
contract GetUnderlyingAssetsAmounts_SlipstreamAM_Fuzz_Test is SlipstreamAM_Fuzz_Test {
    ...
    function testFuzz_Revert_GetUnderlyingAssetsAmounts_Overflow_PriceToken0(
        ...
        ICLPoolExtension pool = createPool(address(token0), address(token1), 1, 1e18, 300);
        ...
        slipstreamAM.getUnderlyingAssetsAmounts(address(creditorUsd), assetKey, 1, new bytes32[](0));
    }
    ...
    function testFuzz_Success_GetUnderlyingAssetsAmounts_AmountIsZero(uint96 tokenId) public {
        ...
        (uint256[] memory underlyingAssetsAmounts, AssetValueAndRiskFactors[] memory rateUnderlyingAssetsToUsd) =
            slipstreamAM.getUnderlyingAssetsAmounts(address(creditorUsd), assetKey, amount, new bytes32[](0));
