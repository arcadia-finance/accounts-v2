...
/**
 * @notice Fuzz tests for the function "setMerklOperators" of contract "AccountV4".
 */
contract SetMerklOperators_AccountV4_Fuzz_Test is AccountV4_Fuzz_Test, MerklFixture {
    ...
    function testFuzz_Success_setMerklOperators_StatusOffToOff(TestParams memory testParams) public {
        ...
        assertEq(distributor.operators(address(account_), address(operator1)), 0);
    }
    ...
    function testFuzz_Success_setMerklOperators_StatusOffToOn(TestParams memory testParams) public {
        ...
        vm.expectCall(
            address(distributor),
            abi.encodeWithSelector(distributor.toggleOperator.selector, address(account_), address(operator1))
        );
        vm.prank(users.accountOwner);
        setMerklOperators(params);

        // And: Operator should be set to status "off".
        assertEq(distributor.operators(address(account_), address(operator1)), 1);
    }

    function testFuzz_Success_setMerklOperators_StatusOnToOff(TestParams memory testParams) public {
        // Given : Operator is set to status "off".
        vm.prank(users.owner);
        distributor.toggleOperator(address(account_), address(operator1));
    ...
    function testFuzz_Success_setMerklOperators_Hook(TestParams memory testParams) public {
        ...
        if (testParams.operatorState2.currentStatus) {
            distributor.toggleOperator(address(account_), address(operator2));
        }
        vm.stopPrank();
        ...
        emit AccountV4.MerklOperatorSet(address(operator1), params.operatorStatuses[0]);
        vm.expectEmit(address(account_));
        emit AccountV4.MerklOperatorSet(address(operator2), params.operatorStatuses[1]);
        vm.prank(users.accountOwner);
        setMerklOperators(params);

        // And: Claim recipient should be set.
        assertEq(distributor.claimRecipient(address(account_), testParams.token1), testParams.recipient);
        assertEq(distributor.claimRecipient(address(account_), testParams.token2), testParams.recipient);
    }
