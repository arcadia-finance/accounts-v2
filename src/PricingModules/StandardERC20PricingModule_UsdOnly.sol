 ...
 */
contract StandardERC20PricingModule_UsdOnly is PricingModule_UsdOnly, IStandardERC20PricingModule {
        ...
        onlyOwner
    {
        ...
        for (uint256 i; i < oraclesLength;) {
            if (oldOracles[i] == decommissionedOracle) {
                require(!IOraclesHub_UsdOnly(oracleHub).isActive(oldOracles[i]), "PM20_SO: Oracle still active");
                // View function, reverts in OracleHub if sequence is not correct.
                IOraclesHub_UsdOnly(oracleHub).checkOracleSequence(newOracles, asset);
