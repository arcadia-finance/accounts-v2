...
import { PricingModule_UsdOnly, IPricingModule_UsdOnly } from "../AbstractPricingModule_UsdOnly.sol";
 ...
 */
contract UniswapV3WithFeesPricingModule_UsdOnly is PricingModule_UsdOnly {
    ...
    PricingModule_UsdOnly immutable erc20PricingModule;
    ...
    constructor(address mainRegistry_, address oracleHub_, address riskManager_, address erc20PricingModule_)
        PricingModule_UsdOnly(mainRegistry_, oracleHub_, 1, riskManager_)
    {
        erc20PricingModule = PricingModule_UsdOnly(erc20PricingModule_);
    }
     ...
     */
    function getValue(IPricingModule_UsdOnly.GetValueInput memory getValueInput)
        public
        view
        override
        returns (uint256 valueInUsd, uint256 collateralFactor, uint256 liquidationFactor)
    {
        ...
        uint256 principal1;
        {
            ...
            (usdPriceToken0,,) = PricingModule_UsdOnly(erc20PricingModule).getValue(
                GetValueInput({ asset: token0, assetId: 0, assetAmount: 1e18, baseCurrency: 0 })
            );
            (usdPriceToken1,,) = PricingModule_UsdOnly(erc20PricingModule).getValue(
...

        {
            ...
            (uint256 collateralFactor0, uint256 liquidationFactor0) =
                PricingModule_UsdOnly(erc20PricingModule).getRiskVariables(token0, baseCurrency);
            (uint256 collateralFactor1, uint256 liquidationFactor1) =
                PricingModule_UsdOnly(erc20PricingModule).getRiskVariables(token1, baseCurrency);
    ...
    function setExposureOfAsset(address asset, uint256 maxExposure) public override {
        ...
        if (exposure[asset].exposure == 0) {
            require(PricingModule_UsdOnly(erc20PricingModule).inPricingModule(asset), "PMUV3_SEOA: Unknown asset");
        }
    }
    ...
    function _getTrustedTickCurrent(address token0, address token1) internal view returns (int256 tickCurrent) {
        // We use the USD price per 10^18 tokens instead of the USD price per token to guarantee
        // sufficient precision.
        (uint256 priceToken0,,) = PricingModule_UsdOnly(erc20PricingModule).getValue(
            GetValueInput({ asset: token0, assetId: 0, assetAmount: 1e18, baseCurrency: 0 })
        );
        (uint256 priceToken1,,) = PricingModule_UsdOnly(erc20PricingModule).getValue(
            GetValueInput({ asset: token1, assetId: 0, assetAmount: 1e18, baseCurrency: 0 })
        );
