 ...
 */
contract ActionMultiCallV10 is IActionBase, ERC721TokenReceiver {
    /* //////////////////////////////////////////////////////////////
                                STORAGE
    ////////////////////////////////////////////////////////////// */
    ...
    function claimAeroRewards(
        ...
        emit FeesPaid(msg.sender, rewardToken, feeCollector, feesPaid);
    }
    ...
    function claimV3Rewards(
        ...
        IERC20(token0).transfer(sendRewardsTo, IERC20(token0).balanceOf(address(this)));
        IERC20(token1).transfer(sendRewardsTo, IERC20(token1).balanceOf(address(this)));

        emit FeesPaid(msg.sender, token0, feeCollector, feesPaid0);
        emit FeesPaid(msg.sender, token1, feeCollector, feesPaid1);
    }
     ...
     * @param feeCollector The address to send the fees to.
     */
    function distributeFees(address[] memory assets, uint256[] memory maxAmounts, address feeCollector) external {
        if (msg.sender != address(this)) revert OnlyInternal();
        ...
        for (uint256 i; i < assets.length; ++i) {
            ...
            if (tokenBalance >= maxAmounts[i]) {
                ...
                emit FeesPaid(msg.sender, assets[i], feeCollector, maxAmounts[i]);
            } else {
                IERC20(assets[i]).transfer(feeCollector, tokenBalance);
                emit FeesPaid(msg.sender, assets[i], feeCollector, tokenBalance);
            }
        }
    }
