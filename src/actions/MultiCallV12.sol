 ...
 */
contract ActionMultiCallV12 is IActionBase, ERC721TokenReceiver {
    /* //////////////////////////////////////////////////////////////
                                STORAGE
    ////////////////////////////////////////////////////////////// */
    ...
    event FeesPaid(address indexed account, address indexed asset, address indexed feeRecipient, uint256 amount);
    ...
    /**
     * @notice Helper function to mint a UniV4 LP token and return the ID for later usage.
     * @param to The contract address of the LP token.
     * @param data The data to call the lp contract with.
     * @dev Asset address and ID is temporarily stored in this contract.
     */
    function mintUniV4LP(address to, bytes memory data) external {
        if (msg.sender != address(this)) revert OnlyInternal();

        uint256 nextTokenId = IPositionManager(to).nextTokenId();
        (bool success,) = to.call(data);
        require(success, "UniV4 Mint Failed");

        mintedAssets.push(to);
        mintedIds.push(nextTokenId);
    }
    ...
    function mintUniV3LPWithConfig(MinimizedMintParams memory mintParams) external {
        if (msg.sender != address(this)) revert OnlyInternal();
        if (mintParams.balance0 == 0 && mintParams.balance1 == 0) {
            mintParams.balance0 = IERC20(mintParams.token0).balanceOf(address(this));
            mintParams.balance1 = IERC20(mintParams.token1).balanceOf(address(this));
        }
        ...
        bytes memory data = abi.encodeWithSelector(
            ...
            INonfungiblePositionManager.MintParams({
                ...
                amount0Desired: mintParams.balance0,
                amount1Desired: mintParams.balance1,
                amount0Min: 0,
                amount1Min: 0,
    ...
    /**
     * @notice Splits a Slipstream LP NFT into multple NFTs
     * @param params The parameters for the tx.
     * @param isStaked Whether the NFT is staked or not.
     * @param isWrapped Whether the NFT is wrapped or not.
     */
    function splitSlipLP(SplitParams memory params, bool isStaked, bool isWrapped) external {
        if (msg.sender != address(this)) revert OnlyInternal();

        if (isStaked && !isWrapped) {
            IStakedSlipstreamAM(0x1Dc7A0f5336F52724B650E39174cfcbbEdD67bF1).burn(params.tokenId);
        } else if (isStaked && isWrapped) {
            IStakedSlipstreamAM(0xD74339e0F10fcE96894916B93E5Cc7dE89C98272).burn(params.tokenId);
        }

        INonfungiblePositionManagerSlip(params.nftManager).decreaseLiquidity(
            INonfungiblePositionManagerSlip.DecreaseLiquidityParams({
        ...
        (uint256 amount0, uint256 amount1) = INonfungiblePositionManagerSlip(params.nftManager).collect(
            INonfungiblePositionManagerSlip.CollectParams({
                tokenId: params.tokenId,
                recipient: address(this),
        ...
        uint256 balance0;
        uint256 balance1;
        for (uint256 i; i < params.splitAmount; ++i) {
            if (i == params.splitAmount - 1) {
                balance0 = amount0 - (amount0 / params.splitAmount * i);
                balance1 = amount1 - (amount1 / params.splitAmount * i);
            } else {
                balance0 = amount0 / params.splitAmount;
                balance1 = amount1 / params.splitAmount;
            }
            data = abi.encodeWithSelector(
                this.mintSlipLPWithConfig.selector,
                MinimizedMintParams({
    ...
    /**
     * @notice Splits a Uniswap V3 NFT into multple NFTs
     * @param params The parameters for the tx.
     */
    function splitUniV3(SplitParams memory params) external {
        if (msg.sender != address(this)) revert OnlyInternal();

        INonfungiblePositionManager(params.nftManager).decreaseLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = INonfungiblePositionManager(params.nftManager).collect(
        ...
        for (uint256 i; i < params.splitAmount; ++i) {
            ...
            data = abi.encodeWithSelector(
                this.mintUniV3LPWithConfig.selector,
                MinimizedMintParams({
                    token0: params.token0,
                    token1: params.token1,
                    fee: params.fee,
    ...
    /**
     * @notice Combines multiple Slipstream LP NFTs into one NFT
     * @param params The parameters for the tx.
     * @param isStaked Whether the NFTs are staked or not.
     * @param isWrapped Whether the NFTs are wrapped or not.
     */
    function combineSlipLP(CombineParams memory params, bool isStaked, bool isWrapped) external {
        if (msg.sender != address(this)) revert OnlyInternal();

        for (uint256 i; i < params.tokenIds.length; ++i) {
            if (isStaked && !isWrapped) {
                IStakedSlipstreamAM(0x1Dc7A0f5336F52724B650E39174cfcbbEdD67bF1).burn(params.tokenIds[i]);
            } else if (isStaked && isWrapped) {
                IStakedSlipstreamAM(0xD74339e0F10fcE96894916B93E5Cc7dE89C98272).burn(params.tokenIds[i]);
            }

            INonfungiblePositionManagerSlip(params.nftManager).decreaseLiquidity(
                INonfungiblePositionManagerSlip.DecreaseLiquidityParams({
                    tokenId: params.tokenIds[i],
                    liquidity: params.liquidities[i],
                    amount0Min: 0,
                    amount1Min: 0,
                    deadline: block.timestamp
                })
            );
            INonfungiblePositionManagerSlip(params.nftManager).collect(
                INonfungiblePositionManagerSlip.CollectParams({
                    tokenId: params.tokenIds[i],
                    recipient: address(this),
                    amount0Max: type(uint128).max,
    ...
    /**
     * @notice Combines multiple Uniswap V3 NFTs into one NFT
     * @param params The parameters for the tx.
     */
    function combineUniV3(CombineParams memory params) external {
        if (msg.sender != address(this)) revert OnlyInternal();
        ...
        for (uint256 i; i < params.tokenIds.length; ++i) {
            ...
            INonfungiblePositionManager(params.nftManager).collect(
                INonfungiblePositionManager.CollectParams({
                    tokenId: params.tokenIds[i],
                    recipient: address(this),
                    amount0Max: type(uint128).max,
     ...
     * @param sendRewardsTo The address to send the rewards to.
     * @param id The id of the Staked Aerodrome/Slipstream LP token.
     * @param feeCollector The address to send the fees to.
     * @param feeShare The fee share to send to the fee collector with 4 decimals precision.
     */
    function claimAeroRewards(
        ...
        IStakedSlipstreamAM(assetModule).claimReward(id);
        uint256 feesPaid = IERC20(rewardToken).balanceOf(address(this)) * feeShare / 10_000;

        IERC20(rewardToken).transfer(feeCollector, feesPaid);
        emit FeesPaid(account, rewardToken, feeCollector, feesPaid);

        uint256 rewards = IERC20(rewardToken).balanceOf(address(this));

        if (sendRewardsTo != account) {
            IERC20(rewardToken).transfer(sendRewardsTo, rewards);
        }

        emit FeesClaimed(account, rewardToken, rewards);
    }

    /**
     * @notice Helper function to wrap an Aerodrome LP token.
     * @param assetAddress The contract address of the NFT Manager.
    ...
    function claimV3Rewards(
        ...
        (bool success, bytes memory result) = address(assetAddress).call(data);
        require(success, string(result));

        uint256 feesPaid0 = IERC20(token0).balanceOf(address(this)) * feeShare / 10_000;
        uint256 feesPaid1 = IERC20(token1).balanceOf(address(this)) * feeShare / 10_000;

        IERC20(token0).transfer(feeCollector, feesPaid0);
        IERC20(token1).transfer(feeCollector, feesPaid1);
        emit FeesPaid(account, token0, feeCollector, feesPaid0);
        emit FeesPaid(account, token1, feeCollector, feesPaid1);

        uint256 rewards0 = IERC20(token0).balanceOf(address(this));
        uint256 rewards1 = IERC20(token1).balanceOf(address(this));

        if (sendRewardsTo != account) {
            IERC20(token0).transfer(sendRewardsTo, rewards0);
            IERC20(token1).transfer(sendRewardsTo, rewards1);
        }

        emit FeesClaimed(account, token0, rewards0);
        emit FeesClaimed(account, token1, rewards1);
    }

    /**
     * @notice Helper function to distribute fees to a fee collector.
     * @param assets The contract addresses of the assets to distribute.
     * @param maxAmounts The maximum amounts of each asset to distribute.
     * @param feeCollector The address to send the fees to.
     */
    function distributeFees(address account, address[] memory assets, uint256[] memory maxAmounts, address feeCollector)
        external
    {
        if (msg.sender != address(this)) revert OnlyInternal();
        ...
        for (uint256 i; i < assets.length; ++i) {
            tokenBalance = IERC20(assets[i]).balanceOf(address(this));
            if (tokenBalance == 0) continue;

            if (tokenBalance >= maxAmounts[i]) {
                IERC20(assets[i]).transfer(feeCollector, maxAmounts[i]);
                emit FeesPaid(account, assets[i], feeCollector, maxAmounts[i]);
            } else {
                IERC20(assets[i]).transfer(feeCollector, tokenBalance);
                emit FeesPaid(account, assets[i], feeCollector, tokenBalance);
            }
        }
    }
