...
contract DeployAerodromeStep1 is Base_Script {
    ...
    constructor() {
        oracleAeroToUsdArr[0] = DeployNumbers.ORACLE_ID_AERO_USD;
    }

    function run() public {
        // Add aero-usd Chainlink oracle.
        bytes memory calldata_ = abi.encodeCall(
            chainlinkOM.addOracle, (DeployAddresses.ORACLE_AERO_USD, "AERO", "USD", DeployNumbers.CUTOFFTIME_AERO_USD)
        );
        addToBatch(ArcadiaSafes.OWNER, address(chainlinkOM), calldata_);

        // Add Aero as Primary asset.
        calldata_ = abi.encodeCall(
            erc20PrimaryAM.addAsset, (DeployAddresses.AERO, BitPackingLib.pack(BA_TO_QA_SINGLE, oracleAeroToUsdArr))
        );
